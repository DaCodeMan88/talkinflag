...
// Update these URLs when Talkin Flag publishes to each platform.
// YouTube is live — others are placeholder links to the search pages.
const platforms = [
  {
    name: "YouTube",
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
          {platforms.map((platform) => (
            <Link
              ...
              className="group flex flex-col items-center gap-3 border border-brand-white/10 bg-[#111111] p-6 hover:border-brand-yellow/40 hover:bg-[#161616] transition-all duration-200"
            >
              <div className="text-brand-white/60 group-hover:text-brand-yellow transition-colors">
                {platform.icon}
              </div>
              <div className="text-center">
                <p className="font-display text-sm uppercase tracking-widest text-brand-white group-hover:text-brand-yellow transition-colors">
                  {platform.name}
                </p>
                <p className="text-brand-white/40 text-xs mt-1 group-hover:text-brand-white/60 transition-colors">
                  {platform.handle}
                </p>
              </div>
              {!platform.live && (
                <span className="text-brand-white/30 text-xs uppercase tracking-widest border border-brand-white/10 px-2 py-0.5">
                  Coming Soon
                </span>
              )}
            </Link>
