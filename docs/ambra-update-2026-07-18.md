...
**3. Roster year — now visible where you'd expect it. Country moved here too.**
Roster year (like name/team/level) is a protected field that goes through admin review to prevent impersonation. That flow existed but was buried. Now the bottom of Edit Profile shows six protected fields with a "Request change" button on each, and a yellow "Pending review" chip once you've submitted. **Country moved into this protected group** — we noticed your profile said "The Netherlands" under Country while your team badge correctly says "Italy National Women's Team." Country feeds your public nationality claim (flag, national-team matching), so it needed the same protection as team/level rather than being a casual free-text field. Note: **you're the admin** — your own requests land in Admin → Change Requests for you or Tika to approve.
...
One small data question while we're at it: your profile currently shows 2 caps — what's the right number (you'd mentioned 24 at some point)? Caps stays self-editable (it just re-triggers verification when changed, which is working correctly), so you can update it yourself in Edit Profile whenever you have the real number.
