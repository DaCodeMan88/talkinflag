...
- **SEO** = Search Engine Optimization (Google et al.). **GEO** = Generative Engine Optimization — being cited by AI answer engines (ChatGPT, Perplexity, Google AI Overviews). **Owner confirmed 2026-08-02: GEO means generative-engine optimization (NOT geographic/local).** Both SEO and GEO are served by the same mechanics: clean structured data, an explicit Q&A/FAQ block, a quotable "key takeaways" summary, clear entity naming, crawlable HTML, and (for GEO) inclusion in `llms.txt`. This plan treats them together.
