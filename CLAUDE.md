...
**🔶 READY TO MERGE 2026-08-01: Assessments 10x (evaluation + Flag IQ + Coach IQ)** — plan `docs/plans/2026-08-01-assessments-10x.md` (branch `assessments-10x`, NOT merged/pushed; tsc clean, **354/354 tests**, `npm run build` green, bias audit exit 0). All 22 tasks done. Addresses Ambra + Coach Jon's "questions/answers all too similar, boring, redundant."
...
- **⚠️ TWO OWNER-GATED STEPS (held on purpose — do NOT run without sign-off):** (1) seed eval v2 live — `npx tsx --env-file=.env.local scripts/seed-eval.ts` — flips /evaluate from the old 50 to the new 28 (prod is STILL v1/50-active until then); (2) re-seed IQ banks — `scripts/seed-iq.ts` + `seed-iq-coach.ts` — applies the content rebalance + `domain` tags (mints new question ids; the runtime shuffle already protects position bias without it). Owner review docs: `docs/eval-bank-v2-review.md` (approve the bank) + confirm Coach IQ keys at `/admin/assessments/questions`. Owner note: `docs/ambra-update-2026-08-01-assessments.md`.
- **Baseline for the next session to prove the 10x:** 0 assessment_sessions rows pre-launch. RLS: 0 policies on eval_items/iq_questions/assessment_sessions/assessment_events (answer key + nonce server-only, verified). **NOT merged, NOT deployed, rankings recompute NOT run.**
