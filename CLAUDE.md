...
**✅ SHIPPED + PUSHED 2026-09-14: Media gallery trim + admin-curated Instagram grid** — plan `docs/plans/2026-09-14-media-gallery-and-ig-curation.md` (read its "EXECUTED" header first: reviews changed the design in six places and the plan's code blocks are stale). Merged to `main` fast-forward, tip `c42eede` (don't poll the deploy). 499/499 tests, tsc clean, build green. Owner note: `docs/ambra-update-2026-09-14.md`.
- **Ambra's 3 requests:** (1) "Behind the mic" (`/hosts-wide.jpg`) removed from `GALLERY_IMAGES` — file KEPT on disk, it's still rendered by `HostsHero.tsx`. (2) The 9 IG reels moved out of code into `media_instagram_posts` so she swaps them monthly at `/admin/media` with no deploy. (3) Her "can I edit the site myself with an AI agent?" answered as a memo in the plan's Appendix A — **not built**; recommendation is to keep converting hardcoded content to admin editors (gallery photos next) + build the read-only assistant from the 2026-08-07 command-center plan.
- **Migration `027_media_instagram_posts.sql` APPLIED LIVE** 2026-09-14 (9 rows seeded, positions 0-8, all `is_live`, `metrics_captured_on` NULL). RLS: one policy, public SELECT where `is_live=true`. Public read still goes through the service-role client (`getLiveInstagramPosts`) so `/media` keeps `revalidate = 300` — a cookie client would force dynamic rendering. The policy is defense-in-depth.
- **`FALLBACK_POSTS`** in `src/lib/media/instagram.ts` is a launch-era snapshot of the seed and does NOT track Ambra's swaps. It renders only on a failed/empty read. If the live grid drifts far from it, refresh it by hand.
- **Three bugs review caught, all fixed:** `parseCount` read `"20.4K"` as 204 AND `"20400 monthly plays"` as 20,400,000,000 (optional K/M suffix ate the first letter of a trailing word) — both now refuse rather than guess; `URL_RE` matched `instagram.com` anywhere in a string so `https://evil.example.com/instagram.com/p/X/` parsed as a real post — now host-anchored via `INSTAGRAM_HOST_PREFIX`; the staleness nag read `updated_at`, which `movePost` stamps on EVERY row, so a reorder silenced it — now reads `metrics_captured_on`.
- **Bundle discipline:** `parseInstagramShortcode` + the host anchor live in `src/lib/media/parse-shortcode.ts` (zero Supabase imports) so the `"use client"` editor can't pull `createAdminClient` into the browser. `instagram.ts` re-exports them; no import site changed. Verified: `SUPABASE_SERVICE_ROLE_KEY` in no client asset.
- **⚠️ NEVER CLICK-TESTED.** `npm run dev` fails in this environment with `EPERM: process.cwd failed … uv_cwd` (same block as prior sessions). Verified via the production build's prerendered HTML instead. **The write paths (add/retire/reorder/inline edit) have never executed against a real table.** Owner-to-verify: one add-and-retire cycle, and the 375px layout of the 4-button rows.
- **Shipped in the same push (9 pre-existing branch commits, NOT reviewed in this session, owner chose to ship them together):** lifecycle emails now fire on claim-received / member-deletion / change-request decisions / claim releases (real mail to real members); `/privacy` gained a **public 30-day response commitment on data requests** — that page is Claude-drafted and still not attorney-reviewed, so confirm with Daniel or licensed counsel before relying on it; admin Members gained the account-signal column; Coach IQ No-Run-Zone question content corrected.

**✅ SHIPPED + PUSHED 2026-08-04: Blog posts moved into the DB + admin delete + dashboard discoverability** — merged to `main` fast-forward, tip `764db56` (don't poll the deploy). tsc clean, 393/393 tests, build green.
- **Why:** Ambra couldn't find the blog feature. It was live, and she IS an admin (`ADMIN_EMAILS` has had her + Tika since 2026-07-05) — but `/admin/blog` existed ONLY as a left-sidebar link, and the sidebar collapses behind a hamburger below `lg`. The `/admin` dashboard's 11 section tiles never mentioned Blog. **Lesson: a new admin surface needs a dashboard tile, not just a sidebar entry.**
- **Dashboard:** Blog tile added 3rd (after Players/Messages) with a live draft/published badge. `sections` gained an optional `badge` so authoring surfaces don't render a bogus "N pending"/"✓ Clear" chip.
- **All 29 launch posts imported into `blog_posts`** via `scripts/import-static-posts.ts` (idempotent by slug — re-running never overwrites an admin's edit; `--dry-run` supported). Ran against prod: 29 published rows, oldest 2025-02-24, newest 2026-06-06, 8 with FAQ. `blog_posts` is now the ONLY post source.
- **`staticPosts` retired to `[]`** (types + `getStaticPostBySlug` kept). Required for delete to mean anything: `mergePostsBySlug` only lets the DB win over code, so a code copy would resurrect any deleted post. Original content lives at `git show 7a5b5c8:src/lib/static-posts.ts`.
- **`BlogTeaser` (homepage) now reads `getAllPostRecords()`** instead of the code array — it was the one consumer that didn't merge DB posts. `revalidateAll()` now also revalidates `/`.
- **`deletePost` action + two-step Delete** in the editor (arm → "Delete forever" + a red warning naming the post and URL). Archive stays the reversible default.
- **Verified on a local production build against prod data:** /blog, feed.xml, sitemap, llms.txt each list all 29; homepage teaser renders 3; **create → edit → delete driven through the real admin UI** using a disposable admin identity (temp auth user + `ADMIN_EMAILS` override + `/auth/confirm?token_hash=` login), all torn down — 0 leftover users, exactly 29 rows.
- **Gotcha for future local admin testing:** `next start` must be restarted so it picks up `.env.local`; `pkill -f "next start"` does NOT match the process — use `kill $(lsof -t -nP -iTCP:<port> -sTCP:LISTEN)`. An `ADMIN_EMAILS=… node …` inline prefix did not reach the server either; putting it in `.env.local` did.
- **Open (not blocking):** imported posts score **2/6** on the editor's SEO checklist (no cover image, no key takeaways) — Ambra can lift each one in the editor. The 5 interview posts still carry `youtube_video_id='TODO_OWNER'` (render path skips it) pending real video IDs.

**✅ SHIPPED 2026-08-03 (`7a5b5c8`) — superseded by the 2026-08-04 entry above. 2026-08-02: DB-backed blog authoring + auto SEO/GEO** — plan `docs/plans/2026-08-02-blog-admin-authoring.md` (branch `blog-and-cleanup`, same branch as the removal below). All 10 tasks done. Admins (Ambra & Tika) can now create/edit/publish blog posts with images from `/admin/blog` — no deploy needed. Owner note: `docs/ambra-update-2026-08-02-blog.md`.
- **Data spine:** `blog_posts` table (migration `025`, applied live via MCP) is a THIRD post source alongside code `staticPosts` + (unconfigured) Sanity. Unified loader `src/lib/blog/posts.ts` (`getAllPostRecords`/`getPostRecordBySlug`/`getPublishedDbPosts`/`toPostRecordFromDb`, tested) merges by slug, DB wins. Public list/detail/sitemap/RSS all serve DB posts through the IDENTICAL render path as static posts (same `RichText`, same Article/Breadcrumb/FAQ JSON-LD) — visually/structurally cohesive. Interview columns added in migration `026` (`youtube_video_id`/`guest_name`/`guest_role`, applied live).
- **Security/RLS:** `blog_posts` has ONE policy — public SELECT limited to `status='published'` (drafts never leak to anon). All writes + draft reads go through the service-role `createAdminClient()` in admin code (verified: no cookie-client `blog_posts` access outside `admin`/`lib/blog`). Every `/admin/blog` page + action + the image route gates via `getAdminUser()`.
- **Editor** (`src/app/admin/blog/`): `BlogEditor.tsx` (title+auto-slug preview, category select, excerpt, markdown-lite body textarea + live `RichText` preview, cover-image UPLOAD to Supabase Storage bucket `blog` via `POST /api/admin/blog/image`, in-body `![alt](url)` images (RichText extended + unit-tested), FAQ + key-takeaways repeaters, guest fields). Server actions `actions.ts` (create/update/publish/unpublish/archive — slug uniqueness w/ `-N` suffix, blank-SEO autofill, `published_at` set only on first publish). `BLOG_CATEGORIES` in `constants.ts`.
- **Auto SEO/GEO** (`src/lib/blog/seo.ts`, tested): `slugify`/`autoMetaDescription`/`autoSeoTitle`/`keyTakeawaysSuggestion`/`seoChecklist`. Editor "SEO & GEO" panel = live char counters, Google + OG previews, live pass/fail checklist, "Reset to auto"/"Suggest from body", read-only JSON-LD preview. Public detail renders cover image + a GEO-quotable "Key Takeaways" block. `/llms.txt` route lists published post URLs (GEO signal). Internal-link suggester `src/lib/blog/links.ts` (tested) + one-click insert in editor; detail page "Related Posts" (same-category, from merged loader).
- **Gate:** tsc clean, **391/391 tests**, `npm run build` green (163 pages incl. `/admin/blog*`, `/api/admin/blog/image`, `/llms.txt`). Public rendering verified via seeded published rows (cover + key-takeaways + FAQ + JSON-LD through RichText) then deleted — `blog_posts` currently **0 rows**. Admin editor browser click-through is auth-gated + the local dev-server preview hit an env EPERM/uv_cwd spawn error, so the interactive admin walkthrough is **owner-to-verify** post-review (build proves all routes compile). Owner preference open: `rel="nofollow"` on affiliate/external body links (not added).
- **Storage:** bucket `blog` (public read) created live. Static posts still render (unchanged); admins can optionally re-create any in the DB later.
...
- **Why it was needed (measured):** all 50 eval items shared ONE stem + Likert scale; in prod 54% of answers = top option, option 0 never once chosen → flat fingerprints → meaningless archetypes (Euclidean classifier let magnitude pick the label). Coach IQ answer key was 88% "B" / 97% "the longest choice" = 97% by guessing. Completion rate was unmeasurable.
- **Spine (live-safe, no seed needed):** `assessment_sessions`+`assessment_events` (migration `020`, applied) = telemetry + a server-only `nonce`. Per-attempt **option shuffle** (`src/lib/assessments/shuffle.ts`) makes position bias unexploitable at serve time; submit re-derives the nonce to map displayed→stored (`src/lib/iq/shuffle-map.ts`, unit-proven both directions). Admin funnel dashboard `/admin/assessments` (starts/completions/completion-%/drop-off histogram/hardest items).
- **Integrity:** `scripts/audit-answer-key.ts` (build gate, exit 1 on biased banks) + content rebalance → banks now uniform (25% per index). `maxPerDimensionFrom` fixed for mixed-dimension items. Archetype now **cosine shape-matching** + "Balanced Evaluator" for flat vectors; `scripts/reclassify-archetypes.ts` already corrected the 5 stored rows (2 bogus "Athlete-First Scout" → "Balanced Evaluator").
- **Typed items** (migration `021`, applied): `item_type ∈ likert|forced_choice|budget|rank|scenario`, one tested pure scorer each (`src/lib/eval/score-types.ts`), dispatched from `scoreFingerprint` (backward-compatible — v1 untyped rows score as likert).
- **New 28-item eval bank v2** (`scripts/data/eval-items-v2.json`, 9-assertion guard test) across 5 formats/5 rounds; runner rebuilt with per-type UI (`src/components/eval/items/*`, budget sliders + rank arrows, 375px-safe) + round checkpoints. IQ: **instant-feedback mode** (`/api/iq/check`, one-check-per-question anti-farming guard) + streaks + test mode; results depth = percentile (suppressed <20 samples) + domain breakdown + eval crowd-deviation. Abandoned-assessment nudge email + daily cron (`/api/cron/assessment-nudge`, once-per-session via `nudged_at`).
- **Admin authoring/review** `/admin/assessments/questions` (migrations `022`+`023`, applied): edit/approve/retire questions, stamps `reviewed_by/at`, advisory bias re-check on save. All 72 IQ questions currently `status='draft'`.
- **Baseline for the next session to prove the 10x:** 0 assessment_sessions rows at go-live. RLS: 0 policies on eval_items/iq_questions/assessment_sessions/assessment_events (answer key + nonce server-only, verified). Rankings recompute NOT run (don't).

**🔶 IN FLIGHT 2026-07-27: Player review workflow + lifecycle emails** — plan `docs/plans/2026-07-27-player-review-workflow-and-lifecycle-emails.md` (branch `player-review-workflow`; tsc clean, 229/229 tests, `npm run build` green; admin/dashboard UI is auth-gated so owner click-through still pending).
- **Root-cause of Ambra's "nudge auto-approved a profile":** NOT a bug. Verified — zero DB triggers, `sendNudge` writes only to `profile_nudges`, and the player (Aleena) was `is_approved=true` since her scrape on 06-25, a month before the 07-23 nudge. Scraped profiles ship publicly-live, so nudging one and seeing it live *read* as approval. Guard test `src/app/admin/members/nudge-isolation.test.ts` locks this in.
- **`players` review state machine** (migration `019_player_review_status.sql`, applied live): `review_status ∈ pending|approved|denied|unreviewed` (backfill: 5 approved, 407 unreviewed) + `denial_reason|denial_note|denial_fix|denied_at|reviewed_by|reviewed_at`. `is_approved` stays the public-visibility gate; `review_status` drives the admin queue.
- **Which email fires when** (all in `src/lib/emails/lifecycle.ts`, pure/tested; sent via `sendEmail`): self-register→`pendingReceivedEmail`; admin approve→`approvedLiveEmail`; admin deny→`deniedEmail` (encouraging: reason + what-to-fix + resubmit, zero "rejected" language); claim approve→`claimApprovedEmail`; nudge→`nudgeEmailHtml` (unchanged). Reason presets + tone rules in `src/lib/review/denial-presets.ts`; state transitions in `src/lib/review/transitions.ts`.
- **Admin:** `/admin/players` Pending tab now filters `review_status='pending'` (was `is_approved=false`, which would leak denied rows); new **Denied** tab; `PendingReviewActions` replaces destructive "Reject=delete" with **Approve** / **Deny (preset + note)** + a subdued "Delete (spam)". `denyPlayer`/`approvePlayer` in `src/app/admin/players/actions.ts`. Members table shows an explicit review-status chip (Live ✓ / Awaiting review / Denied / Imported·unreviewed), separate from the relabeled "Claim pending" chip.
- **Athlete self-serve resubmit:** denied athletes see the reason + one-click resubmit on `/dashboard` (`ResubmitCard.tsx` → `POST /api/players/[id]/resubmit`, which re-checks ownership + `review_status='denied'` then returns the row to `pending` and notifies admins).
- **Owner note to send:** `docs/ambra-update-2026-07-27-review-workflow.md`. **Owner decisions baked as defaults:** denied self-registrations soft-hide (`is_approved=false`, recoverable); denial copy/tone is owner-editable in `lifecycle.ts`/`denial-presets.ts`.
